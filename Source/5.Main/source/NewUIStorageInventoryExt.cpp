...
bool CNewUIStorageInventoryExt::Create(CNewUIManager* pNewUIMng, int x, int y)
{
    ...
    m_pNewUIMng->AddUIObj(INTERFACE_ExpandWarehouse, this);
...
bool CNewUIStorageInventoryExt::UpdateMouseEvent()
{
    ...
    {
        g_pNewUISystem->Hide(INTERFACE_ExpandWarehouse);
        return false;
    }
...
bool CNewUIStorageInventoryExt::InsertItem(int iIndex, BYTE* pbyItemPacket) const
{
	if (m_pNewInventoryCtrl)
	{
		bool AddItem = m_pNewInventoryCtrl->AddItem(iIndex, pbyItemPacket);
		//g_ConsoleDebug->Write(MCD_RECEIVE, "CNewUIStorageInventoryExt::InsertItem %d", AddItem);
