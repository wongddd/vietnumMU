...
bool CNewUIEnterBloodCastle::Create(CNewUIManager* pNewUIMng, int x, int y)
{
	...
	// Enter Button
	int iVal=0;
	for(int i=0 ; i< gProtect.m_MainInfo.CapDo_Blood; i++)
	{
...
bool CNewUIEnterBloodCastle::BtnProcess()
{
	...
	{	
		...
		iItemIndex = g_pMyInventory->FindItemIndex( ITEM_HELPER+18, m_iNumActiveBtn+1 ,TRUE);

		if(iItemIndex == -1)
		{
			iItemIndex = g_pMyInventory->FindItemIndex( ITEM_HELPER+47, -1, TRUE);
		}
