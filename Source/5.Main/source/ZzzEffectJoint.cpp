...
void RenderJoints(BYTE bRenderOneMore)
{
	...
	for (int i = 0; i < MAX_JOINTS; i++)
	{
		...
		{
			// Bỏ qua nếu điều kiện không khớp
			if ((bRenderOneMore == 1 && o->byOnlyOneRender == 2) ||
				(bRenderOneMore == 2 && o->byOnlyOneRender == 1))
				continue;

			// Thiết lập kiểu render
			switch (o->RenderType)
			{
			...
			{
				glColor3fv(o->Light);  // Thiết lập màu
			}

			BindTexture(o->TexType);  // Gán texture
			...
			for (int j = 0; j < (int)o->NumTails; j++)
			{
				...
				// Lưu trữ kết quả tính toán ánh sáng để không phải tính lại nhiều lần
				float Light1, Light2;
				if (o->bTileMapping)
				{
				...
				// Tối ưu hóa bằng cách giảm các phép tính không cần thiết trong vòng lặp
