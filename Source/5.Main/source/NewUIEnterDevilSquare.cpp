...
bool CNewUIEnterDevilSquare::Create(CNewUIManager* pNewUIMng, int x, int y)
{
	...
	// Enter Button
	int iVal=0;
	for(int i=0 ; i< gProtect.m_MainInfo.CapDo_Devil; i++)
	{
...
bool CNewUIEnterDevilSquare::BtnProcess()
{
	...
	{	
		...
		iItemIndex = g_pMyInventory->FindItemIndex( ITEM_POTION+19, 0, TRUE);

		if( iItemIndex == -1 )
		{
			iItemIndex = g_pMyInventory->FindItemIndex( ITEM_POTION+19, m_iNumActiveBtn+1, TRUE);
		}

		if(iItemIndex == -1)
		{
			iItemIndex = g_pMyInventory->FindItemIndex( ITEM_HELPER+46, -1, TRUE);
		}
