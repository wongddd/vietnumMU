...
void CreateFrustrum2D(vec3_t Position)
{
    ...
    {
        Width = (float)GetScreenWidth()/640;//640.0f
    ...
    else
    {
        ...
        {
            Width = (float)GetScreenWidth()/ 640;//640.0f// * 0.1f;
        ...
        else
        {
            ...
            case 0:
				...
				{
					Width = (float)GetScreenWidth()/ 640 * 9.1f * 0.404998f;
				}
				else if(g_Direction.m_CKanturu.IsMayaScene() )
				{
					Width = (float)GetScreenWidth()/500.f * 10.0f * 0.115f;
				}
				else
				{
                ...
                break;
            case 1:
                Width = (float)GetScreenWidth()/500+0.1f;// * 0.1f;
...
void RenderTerrainBlock(float xf,float yf,int xi,int yi,bool EditFlag)
{
	...
	for(int i=0;i<4;i+=lodi)
	{
		...
		{
            if(TestFrustrum2D(xf+0.5f,yf+0.5f,0.f) || CameraTopViewEnable || Camera3DEnable || Camera3DSet || gMapManager.InBattleCastle() == true)
			{
...
void RenderTerrainFrustrum(bool EditFlag)
{
    ...
    for(;yi<=FrustrumBoundMaxY;yi+=4,yf+=4.f)
	{
        ...
        {
            if(TestFrustrum2D(xf+2.f,yf+2.f,g_fFrustumRange) || CameraTopViewEnable || Camera3DEnable || Camera3DSet || gMapManager.InBattleCastle() == true)
			{
#ifdef PJH_NEW_SERVER_SELECT_MAP
				if (gMapManager.WorldActive == WD_73NEW_LOGIN_SCENE)
#else
...
void RenderTerrainFrustrum_After(bool EditFlag)
{
    ...
    for(;yi<=FrustrumBoundMaxY;yi+=4,yf+=4.f)
	{
		...
		for(;xi<=FrustrumBoundMaxX;xi+=4,xf+=4.f)
        {
            if(TestFrustrum2D(xf+2.f,yf+2.f,-80.f) || CameraTopViewEnable || Camera3DEnable || Camera3DSet || gMapManager.InBattleCastle() == true)
			{
