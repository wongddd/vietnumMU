...
namespace giPetManager
{
	...
	void MovePetCommand(CHARACTER* c)
	{
		if (c->m_pPet != NULL)
		{
			...
			{
				Width = 32; Height = 36; x = DisplayWinMid - (AT_PET_COMMAND_END - AT_PET_COMMAND_DEFAULT) * Width / 2 + SkillCount * Width; y = 330;
				SkillCount++;
	...
	bool RenderPetItemInfo(int sx, int sy, ITEM* pItem, int iInvenType)
	{
		...
		{
			int price = 0;
			int indexInv = (MAX_MY_INVENTORY_EX_INDEX) + (pItem->y * COL_PERSONALSHOP_INVEN) + pItem->x;
			char Text[100];
