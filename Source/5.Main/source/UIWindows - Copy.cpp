...
DWORD CUIWindowMgr::AddWindow(int iWindowType, int iPos_x, int iPos_y, const char* pszTitle, DWORD dwParentID, int iOption)
{
	...
	if (!(iOption & UIADDWND_FORCEPOSITION))
	{
		for (m_WindowMapIter = m_WindowMap.begin(); m_WindowMapIter != m_WindowMap.end(); ++m_WindowMapIter)
		{
			...
			{
				//==Fix Wide
				if (iPos_x + pbw->GetWidth() + 20 <= DisplayWin) iPos_x += 20;
				if (iPos_y + pbw->GetHeight() + 20 <= DisplayWin) iPos_y += 20;
				if (iPos_x + pbw->GetWidth() + 20 > DisplayWin && iPos_y + pbw->GetHeight() + 20 > DisplayHeight)
				{
					if (iPos_y % 10 == 9)
					{
						delete pbw;
	...
	pbw->SetPosition(iPos_x, iPos_y);//Set Posion Menu F
...
BOOL CUIBaseWindow::DoMouseAction()
{
	...
	if (GetState() == UISTATE_MOVE)
	{
		if (MouseLButton == true)
		{
			...
			else if (m_iPos_x + m_iWidth + MouseX - m_iMouseClickPos_x > DisplayWin) m_iPos_x = DisplayWin - m_iWidth;
			...
			else if (m_iPos_y + m_iHeight + MouseY - m_iMouseClickPos_y > DisplayHeight)
			{
				m_iPos_y = DisplayHeight - m_iHeight;
			}
			else 
			{
	...
	else if (GetState() == UISTATE_RESIZE)
	{
		if (MouseLButton == true)
		{
			if (m_iResizeDir == 135)
			{
				...
				if (m_iPos_x + m_iWidth + MouseX - m_iMouseClickPos_x > DisplayWin) m_iWidth = DisplayWin - m_iPos_x;
				...
				if (m_iPos_y + m_iHeight + MouseY - m_iMouseClickPos_y > DisplayHeight) m_iHeight = DisplayHeight - m_iPos_y;
...
void CUIPhotoViewer::Init(int iInitType)
{
	...
	if (iInitType > 0)
	{
		//CreateCharacterPointer(&m_PhotoChar, iInitType, (Hero->PositionX), (Hero->PositionY), 0);
		m_PhotoChar = *CreateMonster(iInitType, 0, 0, iInitType + NUMOFMON);
		//m_PhotoChar = &CreateMonster(iInitType, (Hero->PositionX), (Hero->PositionY), DMonster.m_Index + NUMOFMON);
	}
	else
	{
...
void CUIPhotoViewer::UpdateItemChar(int Slot, int IndexItem, int Level, int Ex, int Ancient)
{
	...
	case 5: //helm
	case 6: //helm
		m_PhotoChar.BodyPart[Slot-1].Type = IndexItem;
		m_PhotoChar.BodyPart[Slot-1].Level = Level; //((Level>>3)&15);
		m_PhotoChar.BodyPart[Slot-1].ExtOption = Ancient;
		m_PhotoChar.BodyPart[Slot-1].Option1 = Ex;
		break;
	case 0: //weapon 0
	case 1: //weapon 1
		m_PhotoChar.Weapon[Slot].Type = IndexItem;
		m_PhotoChar.Weapon[Slot].Level = Level; //((Level>>3)&15);
	...
	case 7: //wings
		...
		m_PhotoChar.Wing.Level = Level; //((Level>>3)&15);
	...
	case 8:
		{
			...
			m_PhotoChar.Helper.Level = Level; //((Level>>3)&15);
			...
			//===Custom Pet
			if (gCustomPet2.CheckCustomPetByItem(m_PhotoChar.Helper.Type - MODEL_HELPER))
			{
...
void CUILetterWriteWindow::Init(const char * pszTitle, DWORD dwParentID)
{
	...
	SetPosition(50, 50);  //Window Soan Mail
	SetSize(250, 216);
	SetLimitSize(250, 150);
			...
			case 1:
				{
					...
					dwUIID = g_pWindowMgr->AddWindow(UIWNDTYPE_WRITELETTER, 100,100,temp);//-- Send Mail
					if (dwUIID == 0) break;
...
void CUIFriendMenu::Init()
{
	...
	m_iFriendMenuPos_y = (459+ DisplayHeightExt) - 24;
	m_iFriendMenuHeight = 18;
	SetPosition(582+ DisplayWinExt, m_iFriendMenuPos_y);
