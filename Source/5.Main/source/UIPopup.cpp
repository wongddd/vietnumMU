...
void CUIPopup::UpdateInput()
{
	...
	float fSubWinPos_x = DisplayWinMid - m_sizePopup.cx/2;
...
void CUIPopup::Render()
{
	...
	else
	{
		...
		for( int i=0 ; i<m_nPopupTextCount ; ++i )
		{
			...
			g_pRenderText->RenderText(DisplayWinMid -(size.cx/2), fPosY, m_szPopupText[i], 0, 0, RT3_SORT_LEFT);
			fPosY += 15;
		}

		if( m_PopupType & POPUP_INPUT )
		{
			RenderBitmap( BITMAP_INVENTORY+11, DisplayWinMid -m_nInputSize/2, fPosY, m_nInputSize, 18, 0.f, 0.f, 113.f/128.f, 18.f/32.f );
			...
			if( g_iChatInputType == 1 )
			{
				...
				g_pSingleTextInputBox->SetPosition(DisplayWinMid -m_nInputSize/2+5, fPosY-2 );
		...
		if( m_PopupType & POPUP_TIMEOUT )
		{
			...
			RenderBitmap( BITMAP_INTERFACE_EX+42, DisplayWinMid -75, fPosY, 150.0f, 12.0f ,0.f,0.f, 200.0f/256.0f, 16.0f/16.0f );
			EnableAlphaBlend();
			RenderBitmap( BITMAP_INTERFACE_EX+43, DisplayWinMid -75-4, fPosY, 150.0f*fProgress, 12.0f ,0.f,0.f, (150.0f*fProgress)/256.0f, 16.0f/16.0f );
			DisableAlphaBlend();
			fPosY += 10;
		}
		fPosY += 9;
		if( m_PopupType & POPUP_OK )
		{
			m_OkButton.SetPosition(DisplayWinMid -25, fPosY );
			m_OkButton.Render();
		}
		else if( m_PopupType & POPUP_OKCANCEL )
		{
			m_OkButton.SetPosition(DisplayWinMid -50-10, fPosY );
			m_OkButton.Render();
			m_CancelButton.SetPosition(DisplayWinMid +10, fPosY );
			m_CancelButton.Render();
		}
		else if( m_PopupType & POPUP_YESNO )
		{
			m_YesButton.SetPosition(DisplayWinMid -50-10, fPosY );
			m_YesButton.Render();
			m_NoButton.SetPosition(DisplayWinMid +10, fPosY );
			m_NoButton.Render();
		}
	}
}