...
bool SEASON3B::CNewUIMyShopInventory::Create(CNewUIManager* pNewUIMng, int x, int y)
{
	...
	if (false == m_pNewInventoryCtrl->Create(STORAGE_TYPE::MYSHOP, g_pNewUI3DRenderMng, g_pNewItemMng, this, m_Pos.x + 16, m_Pos.y + 90, 8, 4))
	{
	...
	if (m_pNewInventoryCtrl && pPickedItem && IsRelease(VK_LBUTTON))
	{
		...
		else if (pPickedItem->GetOwnerInventory() == m_pNewInventoryCtrl)
		{
			if (m_pNewInventoryCtrl->CanMove(iTargetIndex, pItemObj))
			{
				ChangeSourceIndex(iSourceIndex);
				ChangeTargetIndex(iTargetIndex);
				SendRequestEquipmentItem(REQUEST_EQUIPMENT_MYSHOP, MAX_MY_INVENTORY_EX_INDEX + iSourceIndex, pItemObj,
					REQUEST_EQUIPMENT_MYSHOP, MAX_MY_INVENTORY_EX_INDEX + iTargetIndex);
				return true;
			}
		}
	}
...
#if(CB_CUSTOMOFFTRADE)
	if (gCB_OffTrade)  gCB_OffTrade->DrawButton(m_Pos.x, m_Pos.y);
...
bool SEASON3B::CNewUIMyShopInventory::Render()
{
	...
	{
		g_pMyShopInventory->SetPos(640 - 190 * 3 + DisplayWinCDepthBox, 0);
	}
	else
	{
		g_pMyShopInventory->SetPos(640 - 190 * 2 + DisplayWinCDepthBox, 0);
	}
	EnableAlphaTest();
