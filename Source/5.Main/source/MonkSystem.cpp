...
void CMonkSystem::RegistItem()
{
	...
	for (std::map<int, CUSTOM_RF_GLOVES>::iterator it = gCustomBattleGloves.m_CustomGlovesInfo.begin(); it != gCustomBattleGloves.m_CustomGlovesInfo.end(); it++)
	{
		if (it->second.ItemIndex != -1)
		{
			...
			m_mapItemEqualType.insert(tm_ItemEqualType::value_type(m_cItemEqualType.GetModelType(), m_cItemEqualType));
			//g_ConsoleDebug->Write(MCD_ERROR, "RegistItem %d", NCount++);
		}
	}
}
...
void CMonkSystem::ModelLinkObject(CHARACTER* _pCha, BYTE _Hand)
{
...
{
	g_ConsoleDebug->Write(MCD_NORMAL, "SetRageSkillAni _nSkill %d", _nSkill);
	switch(_nSkill)
	{
		...
		case AT_SKILL_MUITENPHUONGHOANG:
			SetAction(_pObj, PLAYER_SKILL_MUITENPHUONGHOANG);
			return true;
...
void CMonkSystem::SetDarksideTargetIndex(WORD* _pTargetIndex)
{
	...
	if(!m_nDarksideEffectTotal)
	{
		...
		//int Index = FindCharacterIndex(HeroKey);
		//CHARACTER* c = &CharactersClient[Index];
		//SendDarksideAtt(&c->Object);
	}
	else
	{
...
{
	g_ConsoleDebug->Write(MCD_NORMAL, "RageCreateEffect _nSkill %d", _nSkill);
	switch(_nSkill)
	{
	...
	case AT_SKILL_ATT_UP_OURFORCES:
		{
			SetAction(_pObj, PLAYER_SKILL_HP_UP_OURFORCES);
	...
	case MASTER_SKILL_ADD_GREATER_DEFENSE_SUCCESS_RATE_ENHANCED:
		{
			SetAction(_pObj, PLAYER_SKILL_ATT_UP_OURFORCES);
	...
	case AT_SKILL_MUITENPHUONGHOANG:
	{
		...
		//CHARACTER* c = &CharactersClient[_pObj->m_sTargetIndex];

		//_pObj->Owner = &c->Object;

		//vAngle[2] = CreateAngle(_pObj->Position[0], _pObj->Position[1], c->Object.Position[0], c->Object.Position[1]);

		//Vector(1.0f, 1.0f, 1.0f, vLight);

		//CreateEffect(MODEL_phoenix_shot_effect, _pObj->Position, _pObj->Angle, vLight, 0, _pObj, -1, 0, 0, 0, 1.0, -1);
		...
		CHARACTER* tc = &CharactersClient[_pObj->m_sTargetIndex];
		OBJECT* to = &tc->Object;
		...
		VectorAdd(dp, _pObj->Position, Position);
		Vector(1.f, 1.f, 1.f, Light);
		CreateEffect(MODEL_SKILL_JAVELIN, _pObj->Position, _pObj->Angle, Light, 0, to);
		CreateEffect(MODEL_SKILL_JAVELIN, _pObj->Position, _pObj->Angle, Light, 1, to);
		CreateEffect(MODEL_SKILL_JAVELIN, _pObj->Position, _pObj->Angle, Light, 2, to);

		//CreateEffect(MODEL_phoenix_shot_effect, Position, vAngle, Light, 0);
		PlayBuffer(SOUND_RAGESKILL_DARKSIDE);
	}
	break;
