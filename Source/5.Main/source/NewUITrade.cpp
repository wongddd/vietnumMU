...
bool CNewUITrade::Render()
{
	...
	if (gCB_BotTrader)
	{
		if (!gCB_BotTrader->DrawWindow(m_Pos.x, m_Pos.y))
		{
			if (m_pYourInvenCtrl) m_pYourInvenCtrl->Render();
		}
	}
#else
	if (m_pYourInvenCtrl)
...
void CNewUITrade::ProcessMyInvenCtrl()
{
	...
	if (SEASON3B::IsPress(VK_LBUTTON))
	{
		...
		//gInterface.DrawMessage(1, "0 ProcessMyInvenCtrl Check  (%d ~ %d)", pPickedItem->GetOwnerInventory() == g_pMyInventory->GetInventoryCtrl(), g_pMyInventoryExt->GetOwnerOf(pPickedItem));
		if (pPickedItem->GetOwnerInventory() == g_pMyInventory->GetInventoryCtrl() || g_pMyInventoryExt->GetOwnerOf(pPickedItem)) //Tu nhan vat sang o trade
		{
			int nSrcIndex = pPickedItem->GetSourceLinealPos();
			int nDstIndex = pPickedItem->GetTargetLinealPos(m_pMyInvenCtrl);
			//gInterface.DrawMessage(1, "1 ProcessMyInvenCtrl %d (%d %d)", pItemObj->Type, nSrcIndex, nDstIndex);
										 ...
										 int nTradeIndex)
{
	if (::IsTradeBan(pItemObj))
	{
		g_pChatListBox->AddText(
			"", GlobalText[668], SEASON3B::TYPE_ERROR_MESSAGE);				
		if (IsItemAutoMove())
			SetItemAutoMove(false);
	}
	else
...
void CNewUITrade::ProcessTradeItemAutoMove() //Move Trade -> Inv
{
	if (g_pPickedItem)
	...
	CNewUIInventoryCtrl* pMyInvenCtrl = g_pTrade->GetMyInvenCtrl();
...
//=== Tra KQ MOVE ITEM OK
void CNewUITrade::ProcessTradeItemAutoMoveSuccess()
{
	if (!IsVisible())
		return;
	...
	if (IsItemAutoMove())
	{
		...
		if (pItemObj)
		{
			...
			//sprintf(m_szTextBuff, "Delete Item Trade Box %d %d", MouseX, MouseY);
...
//=== Tra KQ MOVE ITEM LOI
void CNewUITrade::ProcessMyInvenItemAutoMoveFailure()
{
	if (!IsVisible())
		return;
...
void CNewUITrade::ProcessMyInvenItemAutoMove()
{
	if (g_pPickedItem)
		if (g_pPickedItem->GetItem())
			return;
	...
	if (pItemObj)
	{
		...
		if (-1 != nDstIndex && m_pMyInvenCtrl->CanMove(nDstIndex, pItemObj))
		{
			SetItemAutoMove(true);
	...
	//===Auto Move Inv Ext
	CNewUIInventoryCtrl* pMyInvenCtrlExt = g_pMyInventoryExt->GetInventoryCtrl();
	if (!pMyInvenCtrlExt) return;
	ITEM* pItemObjExt = pMyInvenCtrlExt->FindItemAtPt(MouseX, MouseY);
	
	if (pItemObjExt)
	{

		if (pItemObjExt->Type == ITEM_HELPER + 20)
			return;

		int nDstIndex = FindEmptySlot(pItemObjExt);
		if (-1 != nDstIndex && m_pMyInvenCtrl->CanMove(nDstIndex, pItemObjExt))
		{

			SetItemAutoMove(true);
			int nSrcIndexExt= pMyInvenCtrlExt->GetIndexByItem(pItemObjExt);
			//gInterface.DrawMessage(1, "%d %d ~ %d", pItemObjExt->Type, nSrcIndexExt, nDstIndex);
			SendRequestItemToTrade(pItemObjExt, nSrcIndexExt, nDstIndex);
			::PlayBuffer(SOUND_GET_ITEM01);
			return;
...
void CNewUITrade::BackUpYourInven(ITEM* pYourItemObj)
{
    if ((pYourItemObj->Type>=ITEM_HELPER && pYourItemObj->Type<=ITEM_HELPER+4)
		|| (pYourItemObj->Type==ITEM_POTION+13 || gCustomJewel.CheckCustomJewelByItem(pYourItemObj->Type) || pYourItemObj->Type==ITEM_POTION+14 || pYourItemObj->Type==ITEM_POTION+16)
		...
		|| gCustomWing.CheckCustomWingByItem(pYourItemObj->Type) //Custom Wing
...
void CNewUITrade::ProcessToReceiveTradeItems(int nIndex, BYTE* pbyItemPacket)
{
...

	{
		...
		if (IsItemAutoMove())
		{
			...
			if (pMyInvenCtrlExt)
			{
				ITEM* pItemObjExt = pMyInvenCtrlExt->FindItemAtPt(m_nBackupMouseX, m_nBackupMouseY);
				g_pMyInventoryExt->GetInventoryCtrl()->RemoveItem(pItemObjExt);
			}
