...
void SetPlayerStop(CHARACTER *c)
{
	...
	else
	{
		...
		if(o->Type==MODEL_MONSTER01+32 && (TerrainWall[Index]&TW_SAFEZONE)==TW_SAFEZONE)//巴里
...
void RenderCharacter(CHARACTER *c,OBJECT *o,int Select)
{
	...
	for (std::vector<jCEffectMonster::xMonsterEffectBBBBB>::iterator it = gggJCEffectMonster.m_MonsterEffect.begin(); it != gggJCEffectMonster.m_MonsterEffect.end(); it++)
	{
		if (it->IndexMonster == c->MonsterIndex)
		{
			...
			//if (o->BoneTransform[it->EfectValue])
			{
				Vector(it->Color[0], it->Color[1], it->Color[2], v3EffectLightColor);
				Vector(it->Join[1], it->Join[2], it->Join[0], v3EffectPosition);//<<Posicion a partir del Joint

				if (it->TypeEffect == 1)
				{
					//TransformPosition(v554, v531, WorldPos, Bone, 1);
					//b->TransformPosition(o->BoneTransform[it->EfectValue], v3EffectPosition, o->Position, true);
				...
				{
					//TransformPosition(v554, v531, WorldPos, Bone, 1);
					//pPlayStaticEffect1(it->IDEffect, Bone, it->Scale, Color, a2, 0.0, 0); //CreateSprite
...
void RenderCharactersClient()
{
	for ( int i=0; i<MAX_CHARACTERS_CLIENT; ++i )
	{
		...
		if ( o->Live )
		{
			...
			//g_ConsoleDebug->Write(MCD_RECEIVE, "RenderCharactersClient %s %d", c->Object);
		}
...
// 			}
			...
			if(bBack && iType != -1)
			{
				...
				if (o->CurrentAction == PLAYER_FLY || o->CurrentAction == PLAYER_FLY_CROSSBOW) // animation v? c?h
				{
					w->PlaySpeed = 1.f;
				}
				else
