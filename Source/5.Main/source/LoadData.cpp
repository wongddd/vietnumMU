...
inline SpinLock* g_AccessModel_lock = new SpinLock();
inline SpinLock* g_OpenTexture_lock = new SpinLock();
void CLoadData::AccessModel(int Type, char* Dir, char* FileName, int i)
{
	g_AccessModel_lock->lock();
	char Name[255];
	if (i == -1) sprintf(Name, "%s.bmd", FileName);
	else if (i < 10) sprintf(Name, "%s0%d.bmd", FileName, i);
	else sprintf(Name, "%s%d.bmd", FileName, i);
