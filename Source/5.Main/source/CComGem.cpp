...
bool COMGEM::FindWantedList()
{
	...
	bool	bReturn			= false;
	int		nInvenMaxIndex	= MAX_MY_INVENTORY_EX_INDEX;
	ResetWantedList();
	...
	for (int i = 0; i < nInvenMaxIndex; ++i)
	{
		...
		if( isCompiledGem( pItem ) )
		{
			...
			bReturn = true;
			//g_ConsoleDebug->Write(MCD_ERROR, "FindWantedList %d %d (%d) Lv %d", pItem->Type, isCompiledGem(pItem), p.first, p.second);
		}
	}
	return bReturn;		
}
...
bool COMGEM::CheckMyInvValid()
{
	...
	else if(m_bType == DETACH)
	{
		...
		ITEM* pItem = g_pMyInventory->GetInventoryCtrl()->FindItem(iUnMixIndex + MAX_EQUIPMENT);
		if (pItem)
		{
			if (isCompiledGem(pItem))
