...
bool SEASON3B::CNewUIMasterLevel::Create(CNewUIManager* pNewUIMng, int x, int y)
{
	...
	if( WindowWidth != 800 && WindowHeight != 500 )
	{
		...
		if( WindowWidth == 1024 && WindowHeight == 640 )
		{
			panelspaceX = -127;
			panelspaceY = -120;
...
//	float ny = float(429.f * (480.f/(float)WindowHeight));
	...
	m_Width.y = float(540.f * (500.f/(float)640));
	m_Pos.x = (DisplayWin - m_Width.x)/2;
...
void SEASON3B::CNewUIMasterLevel::Render_Icon()
{
	...
	int Loc[4] = {37 + DisplayWinExt,237 + DisplayWinExt,362 + DisplayWinExt,488 + DisplayWinExt };
	int Dir = -1;
