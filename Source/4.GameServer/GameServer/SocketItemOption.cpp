...
void CSocketItemOption::CalcSocketItemOption(LPOBJ lpObj,bool flag) // OK
{
	...
	{
		if (lpObj->EquipInventory[n].IsItem() == 0 || lpObj->EquipInventory[n].IsSocketItem() == 0)
		{
			continue;
		}
		...
		for (int i = 0; i < MAX_SOCKET_ITEM_OPTION_TABLE; i++)
		{
			if (lpObj->EquipInventory[n].m_SocketOption[i] != SOCKET_ITEM_OPTION_NONE && lpObj->EquipInventory[n].m_SocketOption[i] != SOCKET_ITEM_OPTION_EMPTY)
			{
				int ItemOption = lpObj->EquipInventory[n].m_SocketOption[i] % MAX_SOCKET_ITEM_OPTION;
				...
				int ItemOptionLevel = lpObj->EquipInventory[n].m_SocketOption[i] / MAX_SOCKET_ITEM_OPTION;
		...
		if (lpObj->EquipInventory[n].m_SocketOptionBonus != SOCKET_ITEM_BONUS_OPTION_NONE)
		{
			SOCKET_ITEM_BONUS_OPTION_INFO* lpBonusInfo = this->GetBonusInfo(lpObj->Inventory[n].m_SocketOptionBonus);
