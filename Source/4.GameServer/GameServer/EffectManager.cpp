...
void CEffectManager::PeriodicEffect(LPOBJ lpObj,CEffect* lpEffect) // OK
{
	...
	{
		case EFFECT_POISON:
			//damage = (((damage=(int)((lpObj->Life*lpEffect->m_value[2])/100))>lpEffect->m_value[3])?((lpEffect->m_value[3]==0)?damage:lpEffect->m_value[3]):damage); //GOC
		...
		case EFFECT_NEIL:
			...
			break;
		case EFFECT_SAHAMUTT:
			//damage = MAKE_NUMBERDW(lpEffect->m_value[2],lpEffect->m_value[3]); //GOC
			damage = (((damage = (int)((lpObj->Life*lpEffect->m_value[2]) / gServerInfo.SkillDocDW))>lpEffect->m_value[3]) ? ((lpEffect->m_value[3] == 0) ? damage : lpEffect->m_value[3]) : damage);
			absorb = 0;
			effect = 3;
			break;
