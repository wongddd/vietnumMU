...
void CCustomStore::OnPShopBuyItemCallbackRecv(LPOBJ lpObj,LPOBJ lpTarget,DWORD slot,DWORD WCoinC,DWORD WCoinP,DWORD GoblinPoint) // OK
{
	...
	gNotice.NewNoticeSend(lpObj->Index, 0, 0, 0, 0, 0, "Bạn Mua %s của %s Giá %d WC ", gItemManager.GetItemName(lpTarget->Inventory[slot].m_Index), lpTarget->Name, PShopWCCValue);
	#if(GAMESERVER_UPDATE>=501)
