...
#if(CB_BXHDMG)
			info.ShowBXHDmg = lpMemScript->GetAsNumber();
#endif
...
void CCustomMonster::MonsterDieProc(LPOBJ lpObj,LPOBJ lpTarget) // OK
{
	...
	{
		gNotice.GCNoticeSend(lpTarget->Index,1,0,0,0,0,0,gMessage.GetMessage(CustomMonsterInfo.InfoMessage));
	}

	if(CustomMonsterInfo.RewardValue1 > 0  || CustomMonsterInfo.RewardValue2 > 0 || CustomMonsterInfo.RewardValue3 > 0)
	{

		GDSetCoinSend(lpTarget->Index, CustomMonsterInfo.RewardValue1 < 0 ? 0 : CustomMonsterInfo.RewardValue1, CustomMonsterInfo.RewardValue2 < 0 ? 0 :CustomMonsterInfo.RewardValue2, CustomMonsterInfo.RewardValue3 < 0 ? 0 : CustomMonsterInfo.RewardValue3, "CustomMonterAddCoin");

	}
	if (CustomMonsterInfo.RewardValue4 > 0)
	{
		gCashShop.GDCashShopAddPointSaveSend(lpTarget->Index, 0, 0, 0, 0, CustomMonsterInfo.RewardValue4 < 0 ? 0 : CustomMonsterInfo.RewardValue4,"CustomMonterAddCoin");
...
#if(CB_BXHDMG)
bool CCustomMonster::GetShowBXHDmg(int Class, int Map)
{
	for (std::vector<CUSTOM_MONSTER_INFO>::iterator it = this->m_CustomMonsterInfo.begin(); it != this->m_CustomMonsterInfo.end(); it++)
	{
		if (it->Index == Class && (it->MapNumber == -1 || it->MapNumber == Map))
		{
			return it->ShowBXHDmg;
		}
	}
	return 0;
}
